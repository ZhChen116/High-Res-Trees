...
class TensorRegression:
    def __init__(self, a_lam=None, b_lam=None, phi_alpha=None):
        self.a_lam = a_lam
        self.b_lam = b_lam
    ...
    def tensor_reg(self, z_train, x_train, y_train, a_lam=None, b_lam=None, phi_alpha=None, nsweep=1e3, rank=5, burn=0.30,
        ...
        # Set the attributes to allow flexible use of initialization or function call parameters
        self.a_lam = a_lam or self.a_lam
        self.b_lam = b_lam or self.b_lam
        self.phi_alpha = phi_alpha or self.phi_alpha
    ...
    def geterror(self, X_train, y_train, X_test, y_test, burn_in=0.30):
        # Retrieve model parameters from `tensor_reg` output
        if not self.model_output:
            raise ValueError("Model output is missing. Run tensor_reg first.")
        
        beta_store = self.model_output['beta_store']
        sy = self.model_output['sy']
        sx = self.model_output['sx']

        # Calculate Beta_mcmc and Beta_est
        ss = int(len(beta_store) * burn_in)
        Beta_mcmc = self.getBeta_mcmc(beta_store)
        p = [beta_store[0][x].shape[1] for x in range(len(beta_store[0]))]
        Beta_est = (sy / sx) * np.mean(Beta_mcmc[ss:], axis=0).reshape(p)

        # Calculate training error
        train_err = 0
        for i in range(X_train.shape[0]):
            train_err += (np.tensordot(X_train[i], Beta_est, axes=((0, 1, 2), (0, 1, 2))) - y_train[i]) ** 2
        mse_train = train_err / X_train.shape[0]
        rmse_train = mse_train / np.var(y_train)
        
        # Calculate testing error
        test_err = 0
        for i in range(X_test.shape[0]):
            test_err += (np.tensordot(X_test[i], Beta_est, axes=((0, 1, 2), (0, 1, 2))) - y_test[i]) ** 2
        mse_test = test_err / X_test.shape[0]
        rmse_test = mse_test / np.var(y_test)
