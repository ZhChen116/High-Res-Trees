...
class TensorDecisionTreeRegressor:
    def __init__(self, max_depth=2, min_samples_split=2, split_method='variance', split_rank=1, CP_reg_rank=None, Tucker_reg_rank=None, kmeans=None, verbose=0, n_mode=None, const_array = None):
    ...
    def _get_best_split(self, X, y, depth):
        ...
        if optimization_method == 'exhaustive':
            ...
            if self.const_array != None: 
                const_set = set(self.const_array)
                all_indices = np.ndindex(X.shape[1:])
                complement_list = [index for index in all_indices if index not in const_set]
                candidates = np.array(complement_list)
                total_combinations = len(candidates)
                candidates = (tuple(ind) for ind in candidates)
                candidates_list = list(tuple(ind) for ind in candidates)
                if len(candidates_list) == 0:
                    raise ValueError("Candidates list is empty!")
                variances = np.array([np.var(X[(slice(None),) + ind]) for ind in candidates_list])
                p_vecs = variances / np.sum(variances)
                sample_size = max(1, int(self.sample_rate * total_combinations))
                sampled_indices = np.random.choice(len(candidates_list), size=sample_size, p=p_vecs, replace=False)

                # Get the actual sampled candidates based on the sampled indices
                samples = [candidates_list[i] for i in sampled_indices]

                indices = (ind for ind in samples)
            else:
            ...
            # sample_size = max(1, int(self.sample_rate * total_combinations))
            # variances = np.array([np.var(X[(slice(None),) + ind]) for ind in candidates])
            # p_vecs = variances / np.sum(variances)
            # gen_list = list(candidates)
            # samples = np.random.choice(gen_list, size=sample_size, p=p_vecs, replace=False)
            # indices = (ind for ind in samples)
            ...
            #p_vecs = variances / np.sum(variances)
            #if np.isnan(p_vecs).any():
                #print("NaN in p_vecs", p_vecs)
                #raise ValueError("NaN values detected in p_vecs.")
            #if np.isinf(p_vecs).any():
                #print("Inf in p_vecs", p_vecs)
                #raise ValueError("Infinite values detected in p_vecs.")
    
            
            # Add a small epsilon to avoid zero variance
            # variances += 1e-10  # Prevents zero probabilities
            
            # Normalize to probabilities
            # if np.sum(variances) == 0:
            #     print("Warning: All variances are zero.")
            #     p_vecs = np.ones_like(variances) / len(variances)
            # else:
            #     p_vecs = variances / np.sum(variances)
            
            # sample_size = max(1, int(self.sample_rate * total_combinations))
            
            # Ensure p_vecs does not contain too many zeros
            # if np.count_nonzero(p_vecs) < sample_size:
            #     print("Adjusting sample size to avoid too many zero probabilities since this is sampling without replacement.")
            #     sample_size = np.count_nonzero(p_vecs)
            # candidates = (tuple(ind) for ind in complement_list)

            # # Turn the generator into a list for indexed access (since we need to use indices)
            # candidates_list = list(candidates)

            # # Now, select the candidates based on the sampled_indices
            # indices = (candidates_list[i] for i in sampled_indices)


            # sampled_indices = np.random.choice(np.arange(total_combinations), size=sample_size, p=p_vecs, replace=False)
            # #indices = (np.unravel_index(idx, shape_of_interest) for idx in sampled_indices)
    ...
    def apply(self, X):
        ...
        if X.ndim==2:#Only one observation leaf
            ...
            return np.array([self._traverse_tree(X[i,:,:].reshape(-1,self.mode2,self.mode3).squeeze(), self.root, return_leaf_index=True) for i in range(X.shape[0])])
        elif X.ndim==4:
            return np.array([self._traverse_tree(X[i,:,:,:].reshape(-1,self.mode2,self.mode3,self.mode4).squeeze(), self.root, return_leaf_index=True) for i in range(X.shape[0])])
        else:
    ...
    def _traverse_tree(self, x, node, return_leaf_index=True):
        if node is None:
            return 0.
        if node.left is None and node.right is None:
    ...
    def _traverse_tree_with_tucker_regression(self, x, node, depth):
        ...
        if self.split_method=='kmeans':
            ...
            if labels == 0:
                ...
                return self._traverse_tree_with_tucker_regression(x, node.right, depth+1)
    def prune(self, X_val=None, y_val=None, model_type='mean',alpha=0.01,depth=0):
    ...
    def _compute_complexity_measure(self, model_type, depth, alpha=0.01):
